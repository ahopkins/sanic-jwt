...
def test_jwt_ec_crypto_from_path_object(public_ec_key, private_ec_key):
    ...
    sanicjwt = Initialize(
        app,
        authenticate=authenticate,
        secret=public_ec_key,
        secret_key=private_ec_key,
        algorithm='ES256')
...
def test_jwt_rsa_crypto_from_fullpath_as_str(public_rsa_key, private_rsa_key):
    ...
    sanicjwt = Initialize(
        ...
        secret_key=str(private_rsa_key),
        algorithm='RS384')
...
def test_jwt_rsa_crypto_from_str(public_rsa_key, private_rsa_key):
    ...
    sanicjwt = Initialize(
        ...
        secret_key=private_rsa_key.read_text(),
        algorithm='RS512')
...
def test_jwt_ec_crypto_from_str(public_ec_key, private_ec_key):
    ...
    sanicjwt = Initialize(
        app,
        authenticate=authenticate,
        secret=public_ec_key.read_text(),
        secret_key=private_ec_key.read_text(),
        algorithm='ES512')
...
def test_jwt_crypto_wrong_keys():
    ...
    Initialize(
        app,
        authenticate=authenticate,
        secret=str(binascii.hexlify(os.urandom(48)), 'utf-8'),
        secret_key=str(binascii.hexlify(os.urandom(48)), 'utf-8'),
        algorithm='RS256')
